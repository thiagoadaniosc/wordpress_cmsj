		...
		foreach($_POST['mdocs']['tmp-file'] as $index => $tmp) {
			...
			if($valid_mime_type) {
				...
				if(!isset($upload['error'])) {
					...
					$batch_log .= __('File Uploaded with No Errors.','memphis-documents-library')."<br><br>";
				} else {
					$batch_log .= "<b>".__('File Was Not Uploaded because an Error occured.','memphis-documents-library')."</b><br><br>";
				}
			} else {
				$batch_log .= __("Invalid Mime Type => ").$result['type'].__(" Unable to process file.")."<br>";
				$batch_log .= __('File Was Not Uploaded because an Error occured.','memphis-documents-library')."<br><br>";
			} 
			$file = array();
		}
	}
