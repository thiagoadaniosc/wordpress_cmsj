...
function mdocs_nonce() {
	if(get_option('mdocs-disable-sessions') == false && function_exists('session_status')) {
		...
		if(!isset($_COOKIE[md5('mdocs-id-'.get_site_url())])) {
			if(is_ssl()) $the_cookie = setcookie(md5('mdocs-id-'.get_site_url()), $mdocs_init_request, 0, '/', '', true, true);
			else $the_cookie = setcookie(md5('mdocs-id-'.get_site_url()), $mdocs_init_request, 0, '/', '', false, true);
			$mdocs_id = md5($mdocs_init_request.get_site_url());
		} else $mdocs_id = md5($_COOKIE[md5('mdocs-id-'.get_site_url())]);
		session_name('mdocs-'.md5('mdocs-id-'.get_site_url()));
		if(is_ssl()) session_set_cookie_params(0, '/', '', true, true);
		else session_set_cookie_params(0, '/', '', false, true);
		if(session_status() == 2) session_write_close();
		session_id($mdocs_id);
		if(session_status() == 1) session_start();
		if(!isset($_SESSION[md5('mdocs-id'.get_site_url())])) $_SESSION['mdocs-id'] = $mdocs_id;
		if(isset($_SESSION['mdocs-nonce'])) define('MDOCS_NONCE',$_SESSION['mdocs-nonce']);
		if(!isset($_SESSION['mdocs-nonce']) || isset($_REQUEST['mdocs-nonce'])) $_SESSION['mdocs-nonce'] = md5(microtime().rand(0,1000000));
		if(!isset($_SESSION['mdocs-sort-type'])) $_SESSION['mdocs-sort-type'] = null;
		if(!isset($_SESSION['mdocs-order-by'])) $_SESSION['mdocs-order-by'] = null;
	...
	$html = '
			<tr class="mdocs-parent-cat" >
				<td colspan="'.mdocs_get_num_cols().'" id="title" class="mdocs-tooltip">
					'.$tabs['parent-tab'].'<a href="'.$permalink.$parent['slug'].'&mdocs-att='.$att.'" title="'.$permalink.$parent['slug'].'"><i class="fa fa-folder-o"></i> '.$parent['name'].'</a>
				</td>
			</tr>';
	echo $html;
	...
	if(isset($current_cat['children']) && count($current_cat['children']) > 0) {
		foreach($current_cat['children'] as $index => $child) {
			...
			<tr class="mdocs-sub-cats" >
				<td colspan="'.$num_cols.'" id="title" class="mdocs-tooltip">
					'.$tabs['child-tab'].'<a href="'.$permalink.$child['slug'].'&mdocs-att='.$att.'" title="'.$child['name'].'"><i class="fa fa-folder-o"></i> '.$child['name'].'</a>
				</td>
			</tr>';
