			...
			if($id == $the_mdoc['id']) {
				if(is_admin()) { 
					...
					if($contributor == true || $the_mdoc['owner'] == $current_user->user_login || $current_user->roles[0] == 'administrator') {
						...
						<tr>
							<td>
								<input type="text" name="mdocs-batch-edit[<?php echo $index; ?>][name]" class="form-control" placeholder="Name" value="<?php echo $the_mdoc['name']; ?>">
								...
								<div class="mdocs-add-contributor-container" data-contributor-type="batch-edit" data-mdocs-index="<?php echo $index; ?>">
									...
									<div class="mdocs-contributors-container" data-mdocs-index="<?php echo $index; ?>">
										...
										foreach($the_mdoc['contributors'] as $id => $contributor) {
											...
											<button type="button" class="btn btn-sm btn-success" data-mdocs-index="<?php echo $index; ?>" data-contributor-index="<?php echo $id; ?>"><i class="fa fa-user"></i> <?php echo $contributor; ?> <i class="fa fa-times mdocs-contributors-delete-btn" id=""></i></button>
											<?php
										}
										?>
									</div>
